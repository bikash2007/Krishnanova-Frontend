...
const Meditation = () => {
  ...
  useEffect(() => {
    ...
    if (sectionRef.current) sectionRef.current.addEventListener("mousemove", handleMouseMove);
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute w-[600px] h-[600px]" style={{ background: `radial-gradient(circle at center, rgba(251, 191, 36, 0.12) 0%, transparent 50%)`, transform: `translate3d(${mousePosition.x - 300}px, ${mousePosition.y - 300}px, 0)` }} />
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 h-full flex flex-col justify-center">
        ...
        >
          <div className="inline-block text-amber-200 text-xs md:text-sm font-medium tracking-[0.2em] uppercase mb-2">Spiritual Practice</div>
            ...
            <div className="lg:w-4/12 relative bg-gradient-to-br from-amber-400/10 via-purple-400/10 to-indigo-400/10 p-8 flex flex-col justify-center items-center min-h-[200px] lg:min-h-[450px]">
               ...
               <motion.div
                ...
                <div className="absolute inset-0 z-0">
                  <div className="absolute inset-0 opacity-10 animate-[spin_60s_linear_infinite]" style={{ backgroundImage: "radial-gradient(circle, #fbbf24 1px, transparent 1px)", backgroundSize: "20px 20px" }} />
                </div>
            </div>

            {/* RIGHT: Content (Expanded) */}
            <div className="lg:w-8/12 p-6 md:p-10 flex flex-col justify-center">
              <h2 className="text-xl md:text-2xl font-bold text-cyan-200 mb-4">Transform Your Routine</h2>
              <p className="text-sm md:text-lg text-blue-100/80 mb-8 leading-relaxed">
                 Transform your <span className="text-amber-300 font-semibold">Krishnova keychain</span> into a tool for tranquility. 
                 Hold it close, feel the <span className="text-purple-300">divine energy</span>, and center yourself.
              </p>
              ...
              <div className="bg-white/5 rounded-xl p-4 md:p-6 mb-8 border-l-4 border-amber-400">
                 <div className="flex items-center gap-3 mb-2">
                    ...
                    <h3 className="text-sm md:text-base font-bold text-amber-100 uppercase tracking-wide">Morning Mantra</h3>
                 </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 items-center">
                 <div className="flex gap-4 flex-1 w-full overflow-x-auto md:overflow-visible pb-2 md:pb-0">
                    {benefits.map((b, i) => (
                       <div key={i} className="bg-white/5 rounded-lg p-3 min-w-[100px] text-center border border-white/5 flex-1">
                          <div className="text-xl mb-1">{b.icon}</div>
                          <div className="text-xs font-bold text-amber-100">{b.title}</div>
                       </div>
                    ))}
                 </div>
                 <Link to="/productpage" className="w-full md:w-auto">
